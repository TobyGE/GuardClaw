#!/usr/bin/env python3
"""
Generate judge training data by driving OpenCode with curated prompts.
Each prompt triggers real tool calls → GuardClaw hooks → judge.db

Usage:
  python lora/datagen/run-opencode.py                  # run all
  python lora/datagen/run-opencode.py --dry-run         # preview only
  python lora/datagen/run-opencode.py --start 5         # start from #5
  python lora/datagen/run-opencode.py --limit 3         # run only 3
  python lora/datagen/run-opencode.py --dir /tmp/foo    # run in a specific project dir
  python lora/datagen/run-opencode.py --prompts path/to/prompts.jsonl
"""
...
PROMPTS_FILE = os.path.join(SCRIPT_DIR, 'prompts-opencode.jsonl')
DB_PATH = os.path.join(SCRIPT_DIR, '..', 'judge.db')
DEFAULT_DIR = '/tmp/guardclaw-playground-oc-mini'
...
def main():
    parser = argparse.ArgumentParser(description='OpenCode data generator')
    ...
    parser.add_argument('--dir', type=str, default=DEFAULT_DIR, help='Project directory for OpenCode to work in')
    ...
    total = len(prompts)
    before = count_source(DB_PATH, 'opencode')

    _ensure_progress_table(DB_PATH)
    completed = _get_completed(DB_PATH, 'opencode', prompts_basename) if args.resume else set()

    print(f"━━━ OpenCode Data Generator ━━━")
    print(f"Prompts: {prompts_file} ({total} prompts)")
    print(f"Project: {args.dir}")
    print(f"Judge DB: {DB_PATH} (opencode records: {before})")
    ...
    for i, p in subset:
        ...
        _record_run(DB_PATH, 'opencode', prompts_basename, i, p['prompt'], cat, exit_code)
        time.sleep(2)

    after = count_source(DB_PATH, 'opencode')
    new = after - before
    ...
    print(f"New records: {new} (opencode: {before} → {after})")
    print(f"\nNext steps:")
